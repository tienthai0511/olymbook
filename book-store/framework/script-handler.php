...
*/		
		...
		if( strpos($cp_page_xml,'<Product-Slider>') > -1 || strpos($cp_page_xml,'<Featured-Product>') > -1 || strpos($cp_page_xml,'<Best-Saller>') > -1 || strpos($cp_page_xml,'<Blog-Slider>') > -1 || strpos($cp_page_xml,'<Testimonial>') > -1){		
		...
		// Page post_type
			...
			if( strpos($cp_page_xml,'<Featured-Author>') > -1 ){				
					wp_deregister_script('social');
					wp_register_script('social', CP_PATH_URL.'/javascript/social.js', false, '1.0', false);
					wp_enqueue_script('social');		
			}
