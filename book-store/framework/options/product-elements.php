<?php
	...
	*/
	...
	function print_product_best_saller($item_xml){
		...
		echo'     <div class="left"> <span class="title"><a href="' . get_permalink() . '">' . $product_title. '</a></span>'; 
		echo '<span class="author-name">';
		global $post;
		the_terms( $post->ID, 'product_author', 'by ', ', ', ' ' );
		echo '</span>';
		echo '</div>';
	...
	$onsale_category = find_xml_value($item_xml, 'onsale-category');
    wp_reset_query();
	if ($onsale_category == "All") {
	    $onsale_category = "";	
	}
	$post_num = find_xml_value($item_xml, 'onsale-num');
		...
		'product_cat'=>$onsale_category, 'posts_per_page'=>$post_num));
		    $counter_product = 0;
			while( have_posts() ){
							the_post();				
...
////////////Print Product Slider ////////////
	...
	function print_product_slider_item($item_xml){
		...
		$slidercategory = find_xml_value($item_xml, 'category');
		...
		$slidercategory = ($slidercategory == 'All')? '': $slidercategory;
	 ...
	 global $paged;
	 $post_temp = query_posts(array('post_type'=>'product', 'paged'=>$paged, 
			'product_cat'=>$slidercategory, 'posts_per_page'=>$no_slide));
