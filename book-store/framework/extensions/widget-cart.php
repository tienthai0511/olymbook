...
function woocommerce_header_add_to_cartplus_fragment( $fragments ) {
	...
	$translation_cart_item = get_option(THEME_NAME_S.'_translation_cart_item','item');
	
	?>

<a class="cartplus-contents" href="<?php echo $woocommerce->cart->get_cart_url(); ?>" onclick="javascript: return false;" title="<?php _e('View your shopping cart', 'crunchpress'); ?>"><?php echo sprintf(_n('%d '.$translation_cart_item, '%d '.$translation_cart_item, $woocommerce->cart->cart_contents_count, 'woothemes'), $woocommerce->cart->cart_contents_count);?> - <?php echo $woocommerce->cart->get_cart_total(); ?></a>
<?php
...
<div class="cart-contents-toggle"><span class="cart-btn2"> <a></a> </span>
  <?Php if ( $title )
			...
			$translation_cart_item = get_option(THEME_NAME_S.'_translation_cart_item','item');
			$translation_cart_checkout = get_option(THEME_NAME_S.'_translation_cart_checkout','Checkout');
			$translation_cart_viewcart = get_option(THEME_NAME_S.'_translation_cart_viewcart','View Cart');
			
?>
  <div class="clearfix"></div>
  <a class="cartplus-contents" href="<?php echo $woocommerce->cart->get_cart_url(); ?>" onclick="javscript: return false;" title="<?php _e('View your shopping cart', 'woothemes'); ?>"><?php echo sprintf(_n('%d ' .$translation_cart_item, '%d '.$translation_cart_item, $woocommerce->cart->cart_contents_count, 'woothemes'), $woocommerce->cart->cart_contents_count);?> - <?php echo $woocommerce->cart->get_cart_total(); ?></a> </div>
...
<div class="subtotal-buttons">
  ...
  <p class="cartplus-buttons"> <a href="<?php echo $woocommerce->cart->get_cart_url(); ?>" class="button">
    <?php _e($translation_cart_viewcart .' &rarr;', 'woocommerce'); ?>
    </a> <a href="<?php echo $woocommerce->cart->get_checkout_url(); ?>" class="button checkout">
    <?php _e($translation_cart_checkout .' &rarr;', 'woocommerce'); ?>
    </a> </p>
</div>
<?php
		echo $after_widget;
