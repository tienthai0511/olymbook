	...
	*/
	...
	function print_testimonial($item_xml){
		...
		if($display_type == 'Specific Testimonial'){
			...
			$charater_limit = find_xml_value($item_xml, 'charater-limit');
			...
			echo substr($testicontent,0, $charater_limit);
			echo '</div>';
			...
			$item_size = find_xml_value($item_xml, 'item-size');
			$testimonial_category = find_xml_value($item_xml, 'category');
			$testimonial_category = ( $testimonial_category == 'All' )? '': $testimonial_category;
			$testimonial_posts = get_posts(array('post_type'=>'testimonial', 'testimonial-category'=>$testimonial_category, 'numberposts'=>100));
			...
			foreach( $testimonial_posts as $testimonial_post){ 
			  $item_size = '75x75';
			  $thumbnail_id = get_post_thumbnail_id( $testimonial_post->ID );
		  ...
		  <div class="slide">
			   <?php echo '<a href="'.post_permalink( $testimonial_post->ID ).'">'; ?>
				<div class="author-name-holder"> <?php if (!empty ($thumbnail[0])) { echo '<img  src="'. $thumbnail[0] .'" alt="'. $alt_text .'"/>';  } ?> </div></a>
					<strong class="title"><?php echo '<a href="'.post_permalink($testimonial_post->ID).'">' .$testimonial_post->post_title .'</a>'?><span>
					<?php  printf(__('%s','crunchpress'), get_post_meta( $testimonial_post->ID, 'testimonial-option-author-position', true ) );?></span></strong>
					<div class="slide">
				  <p><?php   $testipost_excerpt =  $testimonial_post->post_excerpt;
				              if ( !empty ($testipost_excerpt)) { echo $testipost_excerpt; } else { echo $testimonial_post =  $testimonial_post->post_content; }
