<script>
	jQuery(document).ready(function($){
		if($('#optinforms-form1-container').length > 0){
			//$(this).find('form').append('<input type="hidden" name="format" value="h">');
		}
	});
</script>
<?php
//echo do_shortcode('[optinform]'); 
?>
		<div  style="max-width:1075px;margin:0 auto;">
		<div class="row-fluid ">
			<div class="under-line-head">
				<div class="line-title-head">
					<div class="span4 title-intro text-uppercase">Kiến thức là một tài sản mới</div>
					<div class="span8 feature">
						<ul>
							<li class="text-uppercase book-icon-servise">
								<p>Chuyên đóng sách</p>
								<p>kinh doanh & </p>
								<p>Phát triển bản thân</p>
							</li>
							<li class="text-uppercase book-icon-dis">
								<p>Giao hàng miễn phí</p>
								<p>toàn quốc với</p>
								<p>đơn hàng trên 200k</p>
							</li>
							<li class="text-uppercase book-icon-tel">
								<p>Hot line 24/7</p>
								<p class="hight-light">1900 54 55 56</p>
								<p>chỉ với <sub>1000</sub>Đ/Phút</p>
							</li>
						</ul>
						
					</div>
				</div>
			<div style="clear:both"></div>
			</div>
		<div style="clear:both"></div>
		<div class="span12 witdh-1075 opt-in-content" style="background:#404042">
		<div style="margin:0 auto">
			<?php  echo do_shortcode('[optinmyform]');?>
		</div>
		</div>
		</div>
	</div>
	<div class="clearfix"></div>
	